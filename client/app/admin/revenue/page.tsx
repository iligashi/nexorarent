...
export default function AdminRevenuePage() {
  ...
  if (loading || !data) {
    return (
      <div className="space-y-6">
        <h1 className="text-2xl font-bold text-gray-900 font-outfit">Revenue</h1>
  ...
  return (
    <div className="space-y-6">
      ...
      {/* KPIs */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Revenue" value={formatPrice(totalRevenue)} icon={DollarSign} color="#22C55E" />
        <StatCard label="Total Bookings" value={String(totalBookings)} icon={Calendar} color="#3B82F6" />
        <StatCard label="Avg. Booking Value" value={formatPrice(avgBookingValue)} icon={TrendingUp} color="#FF4D30" />
        <StatCard label="Avg. Rental Days" value={avgDays.toFixed(1)} icon={BarChart3} color="#8B5CF6" />
      </div>
      ...
      {/* Top cars table */}
      <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Top Performing Cars</h2>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100">
                <th className="text-left text-xs font-semibold text-gray-500 uppercase py-3 px-4">#</th>
                <th className="text-left text-xs font-semibold text-gray-500 uppercase py-3 px-4">Car</th>
                <th className="text-left text-xs font-semibold text-gray-500 uppercase py-3 px-4">Bookings</th>
                <th className="text-right text-xs font-semibold text-gray-500 uppercase py-3 px-4">Revenue</th>
              </tr>
            </thead>
            <tbody>
              ...
              {data.top_cars.length === 0 && (
                <tr><td colSpan={4} className="py-8 text-center text-sm text-gray-400">No data available</td></tr>
              )}
            </tbody>
          </table>
        </div>
